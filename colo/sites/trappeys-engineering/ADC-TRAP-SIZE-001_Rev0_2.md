...
| Revision | **0.2** (Approved — rulings applied per decision 0005) |
| Company | ADC Solutions |
| Prepared by | Mission Control — **rulings by Scott Tomsu 2026-06-11** |
| Date | 2026-06-11 |
| Governing inputs | Decisions 0004 + 0005; MGN-BOD-002 Rev 0.2; ADC-CLU-BOM-001 Rev 0.1; ADC-MOD-TCS-001; colo/Heat_Rejection_5MW_IT.pdf; legacy targets per ST-TRAP-BOD-001 Rev 0.6 (reference only) |
...
| Rev | Date | Summary |
|---|---|---|
| 0.1 | 2026-06-11 | First issue. TRAP-OI-03 phase 1: block-count arithmetic, candidate fill rule, first-order hall/cooling/permitting consequences. |
| 0.2 | 2026-06-11 | Scott rulings applied (decision 0005): **fill rule LOCKED at 4× reference cluster** (cluster is the only ADC compute packaging — container-platform comparisons removed); **Stage 1 LOCKED at 19 blocks**. TRAP-OI-04 and TRAP-OI-07 closed. |
...
scan §C). Replaces the legacy containerized-platform arithmetic.

## §2 The N★ block applied to Trappey's [L]
...
**Fill rule [L] (decision 0005):** one block = **4× reference
cluster** (ADC-CLU-BOM-001: 16 racks @ 78 kW = 1,248 kW each):
4 × 1,248 kW = **4,992 kW ≈ 5 MW** (0.16 % slack). Governing
definition: ADC-NSTAR-001.
...
## §3 Block count [L] (Scott ruling 2026-06-11, decision 0005)

| Quantity | Stage 1 | Full Build |
|---|---|---|
| Legacy IT target | 91.1 MW | 182.2 MW |
| **N★ blocks [L]** | **19 → 95.0 MW IT** | **37 → 184.7 MW IT** |
...
ratio is confirmed in ADC-NSTAR-001 against reference-bay (TCS/HAC)
cooling overheads.
...
4 HAC bays, end-fed 2 MW CDU); one block = 4 rows. The legacy set
assigned compute to **B3 and B4** as outdoor containerized units
(NEMA 3R) plus 42,000 + 28,000 sq ft yards — an ADC hall is
conditioned indoor space, so the legacy building math does not
...
3. hall humidity control — the legacy container-coupled Munters/CHP
   regen path died with the legacy platform (scan §A); re-derive
   (with CHP heat use question) in ADC-TRAP-THERM-001.
...
- **Compute payload [O]:** NVL72 allocation remains the gating
  ...
  actual) reconciles in the cluster fill check, ADC-NSTAR-001.
